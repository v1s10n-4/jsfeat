...
import { grayscale, boxBlurGray } from 'jsfeat/imgproc';
...
const demo: Demo = {
  ...
  description: 'Real-time box blur on grayscale image.',
  ...
  process(
    ...
    profiler.start('box blur');
    boxBlurGray(src, dst, radius);
    profiler.end('box blur');
  ...
  onParamChange(key: string, value: unknown) {
    if (key === 'radius') radius = value as number;
  },
