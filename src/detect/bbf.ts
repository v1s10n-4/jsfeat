/**
 * BBF: Brightness Binary Feature object detection.
 *
 * Ported from legacy/jsfeat_bbf.js.
 *
 * Original code is a rewrite from https://github.com/liuliu/ccv
 * @author Liu Liu / http://liuliu.me/
 * @author Eugene Zatepyakin / http://inspirit.ru/
 *
 * The original paper refers to: YEF* Real-Time Object Detection,
 * Yotam Abramson and Bruno Steux
 */

import { Matrix } from '../core/matrix';
import { Pyramid } from '../core/pyramid';
...
export function bbfPrepareCascade(cascade: any): void {
  ...
  for (let j = 0; j < sn; j++) {
    const orig_feature = cascade.stage_classifier[j].feature;
    const f_cnt = cascade.stage_classifier[j].count;
    const feature = cascade.stage_classifier[j]._feature = new Array(f_cnt);
    for (let k = 0; k < f_cnt; k++) {
      feature[k] = {
...
export function bbfBuildPyramid(src: Matrix, minWidth: number, minHeight: number, interval: number = 4): Pyramid {
  ...
  let i = 0, nw = 0, nh = 0;
  let new_pyr = false;
  let src0: Matrix = src, src1: Matrix = src;
  const data_type = DataType.U8 | Channel.C1;

  _interval = interval;
  _scale = Math.pow(2, 1 / (_interval + 1));
  _next = (_interval + 1) | 0;
  _scaleTo = (Math.log(Math.min(sw / minWidth, sh / minHeight)) / Math.log(_scale)) | 0;

  const pyr_l = ((_scaleTo + _next * 2) * 4) | 0;
  ...
  for (i = 1; i <= _interval; ++i) {
    nw = (sw / Math.pow(_scale, i)) | 0;
    nh = (sh / Math.pow(_scale, i)) | 0;
    src0 = img_pyr.data[i << 2];
    if (new_pyr || !src0 || nw != src0.cols || nh != src0.rows) {
      img_pyr.data[i << 2] = new Matrix(nw, nh, data_type);
      src0 = img_pyr.data[i << 2];
    }
    resample(src, src0, nw, nh);
  }
  for (i = _next; i < _scaleTo + _next * 2; ++i) {
    src1 = img_pyr.data[(i << 2) - (_next << 2)];
    src0 = img_pyr.data[i << 2];
    nw = src1.cols >> 1;
    nh = src1.rows >> 1;
    if (new_pyr || !src0 || nw != src0.cols || nh != src0.rows) {
  ...
  for (i = _next * 2; i < _scaleTo + _next * 2; ++i) {
    src1 = img_pyr.data[(i << 2) - (_next << 2)];
    nw = src1.cols >> 1;
    nh = src1.rows >> 1;
    src0 = img_pyr.data[(i << 2) + 1];
    if (new_pyr || !src0 || nw != src0.cols || nh != src0.rows) {
      img_pyr.data[(i << 2) + 1] = new Matrix(nw, nh, data_type);
      src0 = img_pyr.data[(i << 2) + 1];
    }
    ...
    if (new_pyr || !src0 || nw != src0.cols || nh != src0.rows) {
      img_pyr.data[(i << 2) + 2] = new Matrix(nw, nh, data_type);
      src0 = img_pyr.data[(i << 2) + 2];
    }
    ...
    if (new_pyr || !src0 || nw != src0.cols || nh != src0.rows) {
      img_pyr.data[(i << 2) + 3] = new Matrix(nw, nh, data_type);
      src0 = img_pyr.data[(i << 2) + 3];
    }
...
export function bbfDetect(pyramid: Pyramid, cascade: any): BbfRect[] {
  ...
  let i = 0, j = 0, k = 0, n = 0, x = 0, y = 0, q = 0, sn = 0, f_cnt = 0, q_cnt = 0, p = 0, pmin = 0, nmax = 0, f = 0, i4 = 0, qw = 0, qh = 0;
  let sum = 0.0, alpha: any, feature: any, orig_feature: any, feature_k: any, feature_o: any, flag = true, shortcut = true;
  let scale_x = 1.0, scale_y = 1.0;
  const dx = [0, 1, 0, 1];
  const dy = [0, 0, 1, 1];
  const seq: BbfRect[] = [];
  const pyr = pyramid.data;
  const bpp = 1, bpp2 = 2, bpp4 = 4;

  const u8: any[] = [];
  const u8o = [0, 0, 0];
  const step = [0, 0, 0];
  const paddings = [0, 0, 0];
...
export function bbfGroupRectangles(rects: BbfRect[], minNeighbors: number = 1): GroupedRect[] {
  ...
  const node: { parent: number; element: BbfRect | null; rank: number }[] = [];
  for (i = 0; i < n; ++i) {
    node[i] = {
      parent: -1,
      element: rects[i],
